...
(function(){
	...
	//加载设备信息
	DeviceUtil.LoadDeviceData=function(formId,callback){
		var para={};
		BasePage.sendPostRequest(formId,'/deviceController.do?method=ajaxFindDeviceByType',para,function(data){
	...
	DeviceUtil.CaculateDeviceCnt=function(addDevice,gridData){
		if(gridData.length==0){
				//$("#"+tableID).datagrid("appendRow",addDevice[i]);
...
function doOptFormat(value,row,index){
	var add="<img style='padding-top:5px' src='./../images/addCheck.jpg' onClick='javascript:addOne("+index+")'/>";
	var Opt="<input id='"+"OPT_"+index+"' type='text' style='width:15px;height:15px' value='0'/>";
	var substract="<img  style='padding-top:5px;z-index:20000' src='./../images/subtractCheck.jpg' onClick='javascript:substractOne("+index+")'/>";
